...
export function AdDetails({ ad, userId }: AdDetailsParams) {
  ...
  const {
    data: { publicUrl },
  } = supabase.storage.from("avatars").getPublicUrl(`${ad.owner.user_id}.png`);

  const [activeImageIndex, setActiveImageIndex] = useState(0);
  const [isSaved, setIsSaved] = useState(ad.saved_ad.length !== 0);
  const [isInterested, setIsInterested] = useState(
    ad.interested_in_ad.length !== 0
  );
  ...
  return (
    <div className="flex md:flex-row flex-col sm:mx-24 mx-5 mt-5 mb-12 min-h-[85vh] gap-x-20 gap-y-8">
        ...
        <div id="images-carousel" className="relative w-full my-4">
              ...
              <Image
                ...
                src={
                  supabase.storage
                    .from("images")
                    .getPublicUrl(
                      ad.advertisement_picture[activeImageIndex].url
                    ).data.publicUrl
                }
          ...
          <button
            ...
            onClick={() =>
              swapImageToIndex(
                activeImageIndex > 0
                  ? activeImageIndex - 1
                  : ad.advertisement_picture.length - 1
              )
            }
          >
            <span className="inline-flex items-center justify-center w-10 h-10 rounded-full bg-white/30 dark:bg-gray-800/30 group-hover:bg-white/50 dark:group-hover:bg-gray-800/60 group-focus:ring-4 group-focus:ring-white dark:group-focus:ring-gray-800/70 group-focus:outline-none">
              ...
              <span className="sr-only">{i18nCommon("Previous")}</span>
            </span>
          </button>
          <button
            type="button"
            ...
            onClick={() =>
              swapImageToIndex(
                activeImageIndex < ad.advertisement_picture.length - 1
                  ? activeImageIndex + 1
                  : 0
              )
            }
          >
            <span className="inline-flex items-center justify-center w-10 h-10 rounded-full bg-white/30 dark:bg-gray-800/30 group-hover:bg-white/50 dark:group-hover:bg-gray-800/60 group-focus:ring-4 group-focus:ring-white dark:group-focus:ring-gray-800/70 group-focus:outline-none">
              ...
              <span className="sr-only">{i18nCommon("Next")}</span>
            </span>
          </button>
        </div>
        <div className="flex justify-between w-full">
          {ad.status === "Available" ? (
            <Badge variant="default">{i18nCommon("Available")}</Badge>
          ) : ad.status === "Negotiating" ? (
            <Badge variant="secondary">{i18n("NegotiationStarted")}</Badge>
          ) : (
            <Badge variant="destructive">{i18n("NoLongerAvailable")}</Badge>
          )}
      ...
      <div className="lg:w-1/2 w-full flex flex-col gap-y-6 min-h-full justify-center">
        <h4 className="font-semibold text-3xl">{ad.book.title}</h4>
        <span className="font-medium text-2xl">
          € {ad.price < 10 ? ad.price.toPrecision(3) : ad.price.toPrecision(4)}
        </span>
        <table className="w-full">
          <tbody>
            ...
            <tr>
              <td className="font-semibold">{i18n("Author")}</td>
              <td>{ad.book.author}</td>
            </tr>
            <tr>
              <td className="font-semibold">{i18nCommon("Subject")}</td>
              <td>{ad.book.subject}</td>
            </tr>
            <tr>
              <td className="font-semibold">{i18nCommon("Year")}</td>
              <td>{ad.book.year}</td>
            </tr>
            <tr>
            ...
            <tr>
              <td className="font-semibold">{i18nCommon("Conditions")}</td>
              <td>
                {ad.rating === 1 ? (
                  <span>{i18nCommon("Awful")} &#x1F61F;</span>
                ) : ad.rating === 2 ? (
                  <span>{i18nCommon("Mediocre")} &#x1F60C;</span>
                ) : ad.rating === 3 ? (
                  <span>{i18nCommon("Discrete")} &#x1F610;</span>
                ) : ad.rating === 4 ? (
                  <span>{i18nCommon("Good")} &#x1F615;</span>
                ) : (
                  <span>{i18nCommon("Excellent")} &#x1F604;</span>
                )}
              </td>
            </tr>
          </tbody>
        ...
        <div>
          <span className="font-semibold">{i18nCommon("Notes")}</span>
          <ScrollArea className="h-[100px] w-full">{ad.notes}</ScrollArea>
        </div>
        <Button className="mt-8">
          <a className="" href={`mailto:${ad.owner.email}`}>
            {i18n("ContactSeller")}
          </a>
        </Button>
      </div>
