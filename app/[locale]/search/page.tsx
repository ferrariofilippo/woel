...
const FiltersSection = dynamic(
  () =>
    import("@/app/[locale]/search/filters-section").then(
      (mod) => mod.FiltersSection
    ),
  { ssr: false }
);
const AdvertisementDisplay = dynamic(
  () =>
    import("@/components/ui/advertisement-display").then(
      (mod) => mod.AdvertisementDisplay
    ),
  { ssr: false }
);
...
export default function Search({
  ...
  if (typeof query === typeof [""]) query = query[0];

  useEffect(() => {
    supabase.auth.getSession().then((data) => {
      if (!data.data.session) router.push("sign-in");
      else setUserId(data.data.session.user.id);
      ...
      supabase
        ...
        .or(`isbn.like.%${query}%,title.ilike.%${query}%`, {
          foreignTable: "book",
        })
        .order("creation_date", {
          ascending: false,
  ...
  return (
    <div className="mt-5 mb-12 sm:px-24 px-6">
      <FiltersSection setData={setAds} supabase={supabase} userId={userId} />
      <div className="mt-[calc(32px+1rem)]">
        <h4 className="font-semibold text-xl ms-1">{i18n("SearchResults")}</h4>
        <div className="flex sm:flex-row flex-col flex-wrap mt-5">
          {ads && ads.length
            ? ads.map((ad) => (
                <AdvertisementDisplay userId={userId} ad={ad} key={ad.id} />
              ))
            : skeletons.map((s) => (
                <div
                  key={s}
                ...
                >
                  ...
                  <span className="sr-only">{i18nCommon("Loading")}</span>
                </div>
              ))}
        </div>
      </div>
