...
export function FiltersSection({
  ...
  const i18n = useTranslations("Filters");
  const i18nAd = useTranslations("CreateAd");
    ...
    >
      <div className="flex flex-col p-4 gap-4 lg:w-1/2 md:w-2/3 w-full mx-auto">
        <div className="flex gap-2">
          ...
          <h1 className="font-semibold text-xl">{i18nCommon("Filters")}</h1>
        </div>
        <SearchComponent
        ...
        <div className="flex justify-around w-full sm:flex-row flex-col gap-4">
          <Input
            ...
            type="text"
            placeholder={i18nCommon("Subject") + "..."}
          ...
          <Popover>
            <PopoverTrigger asChild>
              ...
              >
                {year ? years.find((y) => y === year) : i18n("ChooseYear")}
              </Button>
            </PopoverTrigger>
        ...
        <div className="grid gap-4">
          <div className="flex items-center justify-between">
            <Label htmlFor="price_slider">{i18n("MaximumPrice")}</Label>
            <span className="rounded-md border border-transparent px-2 py-0.5 text-right text-sm text-muted-foreground hover:border-border">
              € {price < 10 ? price.toPrecision(3) : price.toPrecision(4)}
            </span>
          </div>
          <Slider
            id="price_slider"
